...
# Sample week straight from the real Tips_2024.xlsx sheet V sent:
# week of 2024-07-29, pool 555, 12 staff with their points.
SEED_WEEK = {
    "start_date": "2024-07-29",
    "pool": 555.0,
    "staff": [  # (name, points, vales)
        ("pedro", 52.0, 0.0), ("vitor", 40.0, 0.0), ("xana", 40.0, 0.0),
        ("bruna", 40.0, 0.0), ("rita", 40.0, 0.0), ("didi", 40.0, 0.0),
        ("diogo", 40.0, 0.0), ("felix", 40.0, 0.0), ("marin", 40.0, 0.0),
        ("vitanga", 60.0, 0.0), ("martin", 20.0, 0.0), ("hugo", 20.0, 0.0),
    ],
}
...
def get_week(week_id: int):
    ...
    estaff = conn.execute(
        """SELECT e.id, e.staff_id, s.name, e.points, e.vales
           FROM entries e JOIN staff s ON s.id = e.staff_id
           WHERE e.week_id=? ORDER BY e.points DESC, s.name""",
        (week_id,),
    ).fetchall()
    wk["entries"] = [dict(r) for r in estaff]
    wk["shares"] = _compute_shares(wk["pool_eur"], wk["entries"])
    wk["total_points"] = round(sum(e["points"] for e in wk["entries"]), 1)
    conn.close()
    return wk
...
def save_week(week_id: int, pool_eur: float, entries: list[dict]) -> dict:
    """Replace the week's pool + entries wholesale."""
    conn = _conn()
    ...
    for e in entries:
        conn.execute(
            """INSERT INTO entries (week_id, staff_id, points, vales)
               VALUES (?,?,?,?)""",
            (week_id, e["staff_id"], float(e.get("points", 0) or 0),
...
def _compute_shares(pool_eur: float, entries: list[dict]) -> dict:
    """share_i = pool × points_i / Σpoints, minus vales. Never negative.

    Uses largest-remainder rounding: floor every share to cents, then hand
    the leftover cents one-by-one to the shares with the biggest fractional
    parts, so the gross shares ALWAYS sum to exactly the pool. Without this,
    12 shares rounded independently drift off (47.0338... × 12 ≈ €0.03 lost),
    and the cash never balances.
    """
    total = sum(e["points"] for e in entries)
    if total <= 0:
        return {e["staff_id"]: 0.0 for e in entries}

    # exact gross, floored to cents
    exact = {e["staff_id"]: pool_eur * (e["points"] / total) for e in entries}
    floored = {sid: math.floor(v * 100) / 100 for sid, v in exact.items()}
    leftover_cents = round((pool_eur - sum(floored.values())) * 100)

    # largest remainder first gets the extra cents
